...
- **Spec 144 addendum:** deterministic HLT extrapolation produces `hlt_extrapolation_suggestion.v1` with cap 7 waypoints, one surface per waypoint, `input_hash` freshness, and `suggestion` vs `canonical` promotion via Spec 143 write path only.
